...
function RolesTable() {
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:8000/role/listcreate/');
    ...
    <main className="table-container m-4 h-full rounded-[11px]" style={{ borderRadius: '20px !important', backgroundColor: '#ffffff' }}>
      <Card className="h-full w-full overflow-auto  rounded-[11px]" style={{ borderRadius: '20px !important' }}>
        <div className="ml-8 mr-12 mt-4 mb-8 flex items-center justify-between gap-8">
          ...
          <div className="flex items-center">
            ...
            <div style={{ backgroundColor: '#2185D5', borderRadius: '10px', marginLeft: '8px', display: 'flex', alignItems: 'center' }}>
              <CreateRole open={open} handleClose={handleClose} setOpen={setOpen}>
        ...
        <table className="w-full ml-6 min-w-max table-auto text-left" style={{ fontSize: '14px', fontFamily: 'Poppins', fontWeight: 100 }}>
          <thead>
            <tr>
              {TABLE_HEAD.map((head, index) => (
                <th key={head} className={`border-b border-blue-gray-100 bg-blue-gray-50 p-4 ${index === TABLE_HEAD.length - 1 ? 'flex justify-end pr-28' : ''}`}>
          ...
          <tbody>
          {roles.filter(role => (role.id.toString()).toLowerCase().startsWith(searchQuery.toLowerCase()) || role.name.toLowerCase().startsWith(searchQuery.toLowerCase())).map((role, index) => (
  <tr key={role.id}>
    <td className={`${index === roles.length - 1 ? 'p-4' : 'p-4 border-b border-blue-gray-50'} w-[200px]`}>
    ...
    <td className={`${index === roles.length - 1 ? 'p-4' : 'p-4 border-b border-blue-gray-50'}`}>
      <div className="flex justify-end pr-10">
        <div className="bg-white border border-blue-500 rounded-[6px] w-10 h-10 flex items-center justify-center mr-[4px]" style={{ borderColor: '#D0D3D9' }}>
          <img src={ApprovalIcon} alt="approval" className="h-5 w-5" />
        </div>
